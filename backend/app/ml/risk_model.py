class HealthRiskModel:
    ...
    def predict_risk(self, age, bmi, blood_pressure=None, smoking=False, 
        ...
        # ========== YURAK XAVFI ==========
        heart_risk = 0
        if age > 50: heart_risk += 25
        if bmi > 30: heart_risk += 25
        if smoking: heart_risk += 20
        if family_history: heart_risk += 15
        if blood_pressure:
            try:
                systolic = int(blood_pressure.split('/')[0])
                if systolic > 140: heart_risk += 20
            except:
                pass
        if sleep_quality < 5: heart_risk += 10
        if mood < 5: heart_risk += 5
        ...
        if family_history: diabetes_risk += 20
        if smoking: diabetes_risk += 10
        if sleep_quality < 6: diabetes_risk += 10
        ...
        # ========== GIPERTONIYA ==========
        hypertension_risk = 0
        if age > 40: hypertension_risk += 15
        if bmi > 27: hypertension_risk += 25
        if smoking: hypertension_risk += 15
        if family_history: hypertension_risk += 15
        if sleep_quality < 6: hypertension_risk += 10
        if blood_pressure:
            try:
                systolic = int(blood_pressure.split('/')[0])
                if systolic > 130: hypertension_risk += 25
            except:
                pass
        ...
        # ========== DEPRESSIYA ==========
        depression_risk = 0
        if mood < 4: depression_risk += 40
        if sleep_quality < 4: depression_risk += 30
        if age > 60: depression_risk += 15
        ...
        # ========== UYQU APNOSE ==========
        sleep_apnea_risk = 0
        if bmi > 30: sleep_apnea_risk += 40
        if sleep_quality < 5: sleep_apnea_risk += 30
        if age > 50: sleep_apnea_risk += 20
        if smoking: sleep_apnea_risk += 10
        
        if sleep_apnea_risk > 50:
            recommendations.append("😴 Uyqu apnosi tekshiruvidan o'ting. Vazn kamaytirish va yonboshlab uxlash tavsiya etiladi.")
            risk_score += sleep_apnea_risk

        # ========== JIGAR KASALLIGI ==========
        liver_risk = 0
        if alcohol >= 2: liver_risk += 35
        if bmi > 30: liver_risk += 30
        if alcohol >= 1: liver_risk += 15
        ...
        # ========== ARTRIT ==========
        arthritis_risk = 0
        if age > 50: arthritis_risk += 25
        if bmi > 30: arthritis_risk += 30
        if joint_pain > 0: arthritis_risk += 25
        if joint_pain > 1: arthritis_risk += 20
        ...
        if alcohol >= 2:
            recommendations.append("🍷 Spirtli ichimliklarni kamaytiring. Haftada 1-2 martadan oshirmang.")
    ...
    def get_recommendations(self, risk_level, user_data):
        ...
        if risk_level == "YUQORI":
            recommendations.append({"priority": "yuqori", "text": "Shifokorga murojaat qiling", "deadline": "1 hafta"})
        elif risk_level == "O'RTA":
            recommendations.append({"priority": "o'rta", "text": "3 oy ichida shifokorga murojaat qiling", "deadline": "3 oy"})
        else:
            recommendations.append({"priority": "past", "text": "Yillik tekshiruvlarni davom ettiring", "deadline": "yiliga 1 marta"})
        
        if user_data.get('bmi', 0) > 30:
            recommendations.append({"priority": "yuqori", "text": "Vaznni kamaytirish bo'yicha parhez", "deadline": "darhol"})
